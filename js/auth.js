...
var LocalAuth = (function () {
  ...
  function saveAccounts(a) {
    return Store.set(ACCOUNTS_KEY, a);
  }
  ...
  function writeSession(email, remember) {
    ...
    if (remember) {
      ...
      Cookie.set(SESSION_KEY, JSON.stringify(token), 30);
    } else {
  ...
  return {
    ...
    register: function (name, email, password, remember) {
      ...
      if (!/^[^\s@]+@[^\s@]+\.[a-z]{2,}$/i.test(email))
        return Promise.reject("That email address does not look valid.");
      ...
      return hash(password, salt).then(function (digest) {
        ...
        var list = accounts();
        list.push(account);
        if (!saveAccounts(list))
          return Promise.reject(
            "Your browser blocked local storage, so the account could not be saved.",
          );
      ...
      return hash(password, acc.salt).then(function (digest) {
        ...
        saveAccounts(
          accounts().map(function (a) {
            return a.email === acc.email ? acc : a;
          }),
        );
    ...
    refreshRegistrations: function () {
      return Promise.resolve(LocalAuth.registrations());
    },
...
var CloudAuth = (function () {
  ...
  function friendly(err) {
    ...
    var map = {
      ...
      "auth/unauthorized-domain":
        "This web address is not on the Firebase authorised domains list.",
      "permission-denied":
        "The database rejected that request. Check your Firestore security rules.",
    };
    return (
      map[code] ||
      (err && err.message) ||
  ...
  function profileRef(uid) {
    return window.FB.doc(window.FB.db, "users", uid);
  }
  function regsRef(uid) {
    return window.FB.collection(window.FB.db, "users", uid, "registrations");
  }
  ...
  function loadProfile(user) {
    return window.FB.getDoc(profileRef(user.uid)).then(function (snap) {
      ...
      if (snap.exists()) {
        ...
        profile = base;
        return window.FB.setDoc(profileRef(user.uid), profile).then(
          function () {
            return profile;
          },
        );
  ...
  function loadRegs(uid) {
    return window.FB.getDocs(
      window.FB.query(regsRef(uid), window.FB.orderBy("at", "desc")),
    )
      .then(function (snap) {
        regs = [];
  ...
  return {
    ...
    register: function (name, email, password, remember) {
      ...
      return window.FB.setPersistence(
        window.FB.auth,
        remember
          ? window.FB.browserLocalPersistence
          : window.FB.browserSessionPersistence,
      )
        .then(function () {
          return window.FB.createUserWithEmailAndPassword(
            window.FB.auth,
            email,
            password,
          );
        ...
        .then(function (cred) {
          return window.FB.updateProfile(cred.user, { displayName: name }).then(
            function () {
              var p = {
                uid: cred.user.uid,
                email: email,
              ...
              return window.FB.setDoc(profileRef(cred.user.uid), p).then(
                function () {
                  profile = p;
                  fire(p);
                  return p;
    ...
    login: function (email, password, remember) {
      ...
      return window.FB.setPersistence(
        ...
        .then(function () {
          return window.FB.signInWithEmailAndPassword(
            window.FB.auth,
            email,
            password,
          );
        ...
        .then(function (cred) {
          return loadProfile(cred.user).then(function (p) {
            ...
            return window.FB.updateDoc(profileRef(cred.user.uid), {
              lastLogin: p.lastLogin,
              logins: p.logins,
            }).then(function () {
              profile = p;
              fire(p);
              return loadRegs(cred.user.uid).then(function () {
                return p;
              });
            });
          });
    ...
    logout: function () {
      return window.FB.signOut(window.FB.auth).then(function () {
        profile = null;
        regs = [];
        fire(null);
      });
    ...
    update: function (fields) {
      ...
      return window.FB.updateDoc(profileRef(u.uid), patch)
        .then(function () {
          if (patch.name)
            return window.FB.updateProfile(u, { displayName: patch.name });
        })
        .then(function () {
    ...
    deleteAccount: function () {
      ...
      return window.FB.getDocs(regsRef(uid))
        ...
        .then(function () {
          return window.FB.deleteDoc(profileRef(uid));
        })
        .then(function () {
          return window.FB.deleteUser(u);
        })
        .then(function () {
          profile = null;
          regs = [];
    ...
    addRegistration: function (entry) {
      ...
      return window.FB.addDoc(regsRef(u.uid), rec)
        ...
        .catch(function (e) {
          console.warn("[IntelliLab] could not save registration", e);
          return false;
        });
    },
    ...
    cancelRegistration: function (index) {
      ...
      return window.FB.deleteDoc(
        window.FB.doc(window.FB.db, "users", u.uid, "registrations", target.id),
      )
        .then(function () {
          regs.splice(index, 1);
          document.dispatchEvent(new CustomEvent("registrations:changed"));
        ...
        .catch(function (e) {
          console.warn("[IntelliLab] could not cancel", e);
          return false;
        });
    },
...
var Auth = (function () {
  ...
  return {
    ...
    sessionKind: function () {
      return backend.sessionKind();
    },
    register: function (n, e, p, r) {
      return backend.register(n, e, p, r);
    },
    login: function (e, p, r) {
      return backend.login(e, p, r);
    },
    logout: function () {
      return backend.logout();
    },
    update: function (f) {
      return backend.update(f);
    },
    deleteAccount: function () {
      return backend.deleteAccount();
    },
    addRegistration: function (x) {
      return backend.addRegistration(x);
    },
    registrations: function () {
      return backend.registrations();
    },
    refreshRegistrations: function () {
      return backend.refreshRegistrations();
    },
    cancelRegistration: function (i) {
      return backend.cancelRegistration(i);
    },
    accountCount: function () {
      return backend.accountCount();
    },
    isReady: function () {
      return backend.kind === "local" || backend.isReady();
    },
    initials: initialsOf,
  };
})();
  ...
  paint: function () {
    ...
    if (user) {
      ...
      document.querySelectorAll("[data-user-name]").forEach(function (el) {
        el.textContent = String(user.name).split(" ")[0];
      });
  ...
  init: function () {
    ...
    if (modal) {
      modal.addEventListener("click", function (e) {
        if (e.target === modal || e.target.closest("[data-close-auth]"))
          AuthUI.closeModal();
      });
    }
    ...
    document.querySelectorAll("[data-auth-tab]").forEach(function (b) {
      b.addEventListener("click", function () {
        AuthUI.showTab(b.getAttribute("data-auth-tab"));
      });
    });
        ...
        Promise.resolve(Auth.logout()).then(function () {
          toast(
            "Signed out" +
              ...
              ". Your shortlist stays on this device.",
            "info",
          );
      ...
      signin.addEventListener("submit", function (e) {
        ...
        Auth.login(
          ...
          .then(function (acc) {
            ...
            signin.reset();
            toast(
              "Welcome back, " + String(acc.name).split(" ")[0] + ".",
              "ok",
            );
          })
          .catch(function (msg) {
    ...
    if (reg) {
      ...
      if (pw && meter) {
        pw.addEventListener("input", function () {
          ...
          meter.nextElementSibling.textContent = v
            ? labels[score]
      ...
      reg.addEventListener("submit", function (e) {
        ...
        Auth.register(
          ...
          .then(function (acc) {
            ...
            reg.reset();
            toast(
              "Welcome to IntelliLab, " +
                String(acc.name).split(" ")[0] +
...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (mode === "local" && window.FIREBASE_READY) {
      log(
        "Firebase is configured but still loading — will switch over when ready.",
      );
    }
  });

  function upgradeToCloud(why) {
    if (!uiReady || Auth.isCloud() || !(window.FB && window.FB.auth))
      return false;
    Auth.use("cloud");
    AuthUI.paintMode();
    AuthUI.paint();
  ...
  window.addEventListener("load", function () {
    ...
    var timer = setInterval(function () {
      tries++;
      if (
        upgradeToCloud("detected after " + tries + " check(s)") ||
        tries >= 20
      ) {
        clearInterval(timer);
        if (!Auth.isCloud() && tries >= 20) {
          console.warn(
            "[IntelliLab] Firebase was configured but never became available. " +
              "Accounts are running in browser-only fallback mode. " +
              (window.FB_STATUS
                ? window.FB_STATUS.reason
                : "No status was reported — check for a script error above."),
          );
        }
      }
    }, 250); // up to 5 seconds
