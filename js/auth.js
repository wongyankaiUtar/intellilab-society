...
var LocalAuth = (function () {
  ...
  return {
    ...
    /* Local accounts are read straight out of the browser, so the sign-in
       state is known the instant this file runs. */
...
var Auth = (function () {
  ...
  return {
    ...
    /* ------------------------------------------------------------------
       Firebase restores an existing session over the network, so for the
       first few hundred milliseconds of a page load isSignedIn() returns
       false even for a member who IS signed in. Any code that acts on the
       visitor being a guest — opening the sign-in dialog, redirecting away,
       hiding member content — must wait for the state to settle first, or
       it acts on a false negative.

       Callback receives true if somebody is signed in. The cap stops the
       page hanging for ever if the network never answers.
       ------------------------------------------------------------------ */
    whenSettled: function (cb, capMs) {
      var cap = typeof capMs === 'number' ? capMs : 5000;
      var waited = 0, step = 100;
      (function poll() {
        if (backend.current()) { cb(true); return; }        // known signed in
        if (Auth.isReady() || waited >= cap) { cb(!!backend.current()); return; }
        waited += step;
...
var AuthUI = {
  ...
  init: function () {
    ...
    document.addEventListener('auth:changed', function (e) {
      AuthUI.paint();
      /* Safety net. If the sign-in dialog was opened while Firebase was still
         restoring a session, close it as soon as we learn somebody is in. */
      if (e && e.detail) AuthUI.closeModal();
